...
export default {
  ...
  setup() {
    ...
    const clickAdd = () => {
      ...
      post_addTag(dataInput.value).then(() => getData())
    }
    const clickUpdate = () => {
      if (dataInput.value.name == null) {
        alert('chưa nhập giá trị')
      ...
      post_updateTag(dataInput.value).then(() => getData())
    }
    const clickDeleteCatergory = (idTag) => {
      const check = confirm('bạn muốn xóa tag này id ' + idTag)
      if (!check) {
        return
      }
      post_deleteTag(idTag).then(() => getData())
    }
